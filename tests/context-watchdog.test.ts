...
import setupWatchdog, {
  ...
  shouldCompactNow,
  thresholdPercent,
} from "../extensions/context-watchdog/index.ts";

describe("thresholdPercent", () => {
  it("defaults to 80 when unset", () => {
    expect(thresholdPercent({})).toBe(80);
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "  " })).toBe(80);
  });

  it("honors a valid override", () => {
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "70" })).toBe(70);
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "92.5" })).toBe(
      92.5,
    );
  });

  it("falls back to the legacy LITTLE_CODER_* names", () => {
    expect(thresholdPercent({ LITTLE_CODER_COMPACT_AT_PERCENT: "70" })).toBe(
      70,
    );
    expect(thresholdPercent({
      SMALL_CODER_COMPACT_AT_PERCENT: "60",
      LITTLE_CODER_COMPACT_AT_PERCENT: "70",
    })).toBe(60);
  });

  it("treats non-numeric as the default", () => {
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "soon" })).toBe(
      80,
    );
  });

  it("disables for out-of-band values (<=0 or >=100)", () => {
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "0" })).toBe(0);
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "-5" })).toBe(0);
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "100" })).toBe(0);
    expect(thresholdPercent({ SMALL_CODER_COMPACT_AT_PERCENT: "150" })).toBe(0);
  });

  it("hard-off via SMALL_CODER_NO_COMPACT_WATCHDOG=1 overrides a percent", () => {
    expect(
      thresholdPercent({
        SMALL_CODER_NO_COMPACT_WATCHDOG: "1",
        SMALL_CODER_COMPACT_AT_PERCENT: "70",
      }),
    ).toBe(0);
  });
});
...
describe("shouldCompactNow", () => {
  ...
  it("no-ops on unknown token usage (null right after compaction)", () => {
    expect(shouldCompactNow(usage({ tokens: null, percent: null }), 80, false))
      .toBe(false);
  });
  ...
  it("no-ops when disabled (pct<=0) or usage missing / window unknown", () => {
    ...
    expect(shouldCompactNow(usage({ contextWindow: 0 }), 80, false)).toBe(
      false,
    );
  });

  it("after compaction, the run is resumed (not stranded at the prompt)", async () => {
    // pi's threshold compaction aborts the run and does NOT auto-continue, so
    // without a resume message the task would stall at the prompt.
  ...
  it("pauses instead of looping when a compaction frees too little, then re-arms on recovery", async () => {
    const env = process.env.SMALL_CODER_COMPACT_AT_PERCENT;
    process.env.SMALL_CODER_COMPACT_AT_PERCENT = "80";
    try {
      const handlers: Record<string, Function> = {};
      ...
      // The loop shape: after compaction the resumed run re-reads files and
      // usage is STILL over threshold. A blind watchdog would fire compact()
      // again into a futile compaction; the guard must pause instead.
      let percent = 92;
      const compact = vi.fn();
      const notify = vi.fn();
      const ctx = {
  ...
  it("a failed compaction (onError) pauses rather than silently retrying", async () => {
    const env = process.env.SMALL_CODER_COMPACT_AT_PERCENT;
    process.env.SMALL_CODER_COMPACT_AT_PERCENT = "80";
    try {
      const handlers: Record<string, Function> = {};
  ...
  it("a run climbing 34k→64k on a 64k window compacts before overflow", () => {
    const window = 64000;
