...
//
// History is stored in <agentDir>/small-coder-prompt-history.json so a brand
...
function historyFile(): string {
  return join(agentDir(), "small-coder-prompt-history.json");
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    if (
      !ctx.hasUI || typeof (ctx.ui as any).setEditorComponent !== "function"
    ) return;
    store.reset();
