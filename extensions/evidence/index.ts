...
// Per-session in-memory store of evidence entries. Research tasks require
// cite-before-answer, and these entries survive compaction (the
// evidence-compact extension preserves them).
...
function sessionKey(): string {
  return process.env.SMALL_CODER_SESSION_ID ||
    ...
    async execute(_id, { id }) {
      ...
      if (!e) {
        return {
          content: [{
            type: "text",
            text: `Error: evidence id '${eid}' not found`,
          }],
          details: {},
          isError: true,
        };
